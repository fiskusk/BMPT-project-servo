...
void setup()
{
	...
	/* Setup for TIMER1 - MOD 15 (WGM11, WGM10, WGM13) -> FAST PWM, 
	* for MOD 9 (WGM10, WGM13) -> PWM, Phase correct,
	* clear OCR1B on conpare match and set output to low level (COM1B1). This register set pulse width. 
	* set divider to 8bits (CS11) tick = 0,5 us */
...
ISR(INT0_vect)
{	/*
	if (bit_is_clear(PINB,1))		// if PINB1 = 0 -> increment value d 
	{
		if (d<2100){					
			d += step;				// if not achieve high limit edge -> increment value d
			if (d>2100)
				d = 2100;
			}
		else{
			d = 2100;				// if achieve high limit edge -> fill constant d fixed value of high limit and don't increment
			}
	}
	else
	{								// if PINB1 = 1 -> decrement value d
		if (d>615){				
			d -= step;				// if not achieve low limit edge -> decrement value d
			if (d<615)
				d = 615;
			}
		else{
